...
def _fetch_bgc_zone(zone: pd.Series) -> pd.DataFrame | None:
...
def fetch_bgc_forecast(zones: pd.DataFrame) -> pd.DataFrame:
    frames: list[pd.DataFrame] = []

    with ThreadPoolExecutor(max_workers=CMEMS_MAX_WORKERS) as executor:
        futures = {executor.submit(_fetch_bgc_zone, zone): zone["zone_id"] for _, zone in zones.iterrows()}
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"[WARN] Échec récupération BGC pour zone {zone_id}: {exc}")
