...
def normalize_rule_metric(value: Any) -> str:
    if value is None:
        return ""
    try:
        numeric = float(value)
...
def rule_zone_code(seed_rule: dict[str, Any]) -> str:
    zone = seed_rule.get("zone") or {}
    return str(zone.get("zone_code") or "")


def rule_version_fingerprint(seed_rule: dict[str, Any]) -> str:
    parts = [
        seed_rule.get("rule_type"),
        seed_rule.get("activity_type"),
        seed_rule.get("constraint_type"),
        seed_rule.get("title"),
        seed_rule.get("description"),
        seed_rule.get("legal_reference"),
        seed_rule.get("metric_type"),
        normalize_rule_metric(seed_rule.get("metric_value")),
        seed_rule.get("metric_unit"),
        seed_rule.get("species_common_name"),
        seed_rule.get("species_scientific_name"),
        rule_zone_code(seed_rule),
        seed_rule.get("valid_from"),
        seed_rule.get("valid_to"),
    ]
    normalized = "|".join(normalize_rule_text(part) for part in parts)
    return hashlib.sha256(normalized.encode("utf-8")).hexdigest()


def rule_version_payload(seed_rule: dict[str, Any]) -> dict[str, Any]:
    citation = primary_citation(seed_rule)
    candidate = seed_rule.get("candidate") or {}
    return {
        "rule_type": seed_rule.get("rule_type"),
        "activity_type": seed_rule.get("activity_type"),
        "constraint_type": seed_rule.get("constraint_type"),
        "title": seed_rule.get("title"),
        "description": seed_rule.get("description"),
        "legal_reference": seed_rule.get("legal_reference"),
...
def upsert_audit_source_document(
    ...
    checked_at = citation.get("checked_at") or source.get("checked_at") or timestamp_iso
    fetched_at = citation.get("fetched_at") or source.get("fetched_at") or timestamp_iso
    ...
    data = {
        ...
        "content_length": citation.get("content_length") or source.get("content_length") or len(quote),
...
def upsert_source_candidates(client, source_candidates: list[dict[str, Any]], timestamp_iso: str) -> int:
    count = 0
    for candidate in source_candidates:
        ...
        if not source_url:
            continue
        data = {
            "candidate_key": str(candidate.get("id") or hashlib.sha1(source_url.encode("utf-8")).hexdigest()[:10]),
            "source_url": source_url,
            "canonical_url": candidate.get("canonical_url") or source_url,
            ...
            "ai_relevant": candidate.get("ai_relevant"),
        ...
        client.table("reg_source_candidates").upsert(data, on_conflict="candidate_key").execute()
        count += 1
    return count
...
def sync_rule_version(
    ...
    fingerprint = rule_version_fingerprint(seed_rule)
    rule_key = str(seed_rule["rule_key"])
    citation = primary_citation(seed_rule)
    document_hash = citation.get("document_hash") or (seed_rule.get("source") or {}).get("document_hash")

    current_response = (
        client.table("reg_rule_versions")
        .select("id,rule_fingerprint")
    ...
    stale_ids = [
        row["id"]
        for row in (current_response.data or [])
        if str(row.get("rule_fingerprint") or "") != fingerprint
    ]
    if stale_ids:
    ...
    data = {
        ...
        "status": seed_rule.get("status") or ("needs_review" if seed_rule.get("needs_manual_review") else "published"),
        "confidence_score": seed_rule.get("confidence_score"),
        "extracted_payload": rule_version_payload(seed_rule),
        "citations": seed_rule.get("citations") or [citation],
        "last_seen_at": timestamp_iso,
    ...
    response = client.table("reg_rule_versions").upsert(
        data,
        on_conflict="rule_key,rule_fingerprint",
    ).execute()
    rows = response.data or []
    if not rows:
...
def main() -> None:
    ...
    metadata = {
        ...
        "source_candidates_file": str(SOURCE_CANDIDATES_PATH),
    ...
    try:
        ...
        source_candidates = load_optional_json_list(SOURCE_CANDIDATES_PATH)
