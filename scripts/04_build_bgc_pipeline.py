...
MARGIN_DEG = 0.20
OUT_FILE = FEATURES_DIR / f"features_bgc_daily_by_spot_{YEARS_LABEL}.parquet"
OUT_SAMPLE_FILE = FEATURES_DIR / "features_bgc_daily_by_spot_sample.parquet"
OUT_SUMMARY_FILE = FEATURES_DIR / "features_bgc_daily_by_spot_summary.csv"
...
def download_original_files() -> None:
    ...
    copernicusmarine.get(dataset_id=DATASET_ID, output_directory=str(RAW_BGC_DIR), filter=YEAR_FILTER, overwrite=False, disable_progress_bar=False)
...
def maybe_select_surface(ds: xr.Dataset) -> xr.Dataset:
    for depth_name in ["depth", "depthu", "depthv", "deptht", "lev"]:
        if depth_name in ds.dims or depth_name in ds.coords:
            try: ds = ds.isel({depth_name: SURFACE_DEPTH_INDEX})
...
def open_and_crop_bgc(nc_files: list[Path], lon_min: float, lon_max: float, lat_min: float, lat_max: float) -> xr.Dataset:
    if not nc_files: raise FileNotFoundError("Aucun fichier .nc trouvé pour BGC")
    datasets: list[xr.Dataset] = []
    total = len(nc_files)
    print(f"\n[BGC] {total} fichiers à ouvrir...")
    for i, file_path in enumerate(nc_files, start=1):
        t0 = time.time(); print(f"[BGC] [{i}/{total}] ouverture: {file_path.name}", flush=True)
        try:
            ...
            if not picked:
                print(f"[BGC] [{i}/{total}] aucune variable utile trouvée", flush=True); ds.close(); continue
            ...
            if ds.sizes.get("lat", 0) == 0 or ds.sizes.get("lon", 0) == 0:
                print(f"[BGC] [{i}/{total}] vide après découpe", flush=True); ds.close(); continue
            print(f"[BGC] [{i}/{total}] OK en {time.time() - t0:.1f}s | vars={list(ds.data_vars)} | dims={dict(ds.sizes)}", flush=True)
            datasets.append(ds)
        except Exception as exc:
            print(f"[BGC] [{i}/{total}] ERREUR en {time.time() - t0:.1f}s : {exc}", flush=True)
    if not datasets: raise ValueError("Aucun fichier exploitable pour BGC")
    print(f"[BGC] concaténation de {len(datasets)} sous-datasets...", flush=True)
    ds_merged = xr.concat(datasets, dim="time").sortby("time").drop_duplicates(dim="time")
    print(f"[BGC] concat terminé | dims={dict(ds_merged.sizes)}", flush=True)
    return ds_merged
...
def extract_one_spot(ds: xr.Dataset, spot: pd.Series) -> pd.DataFrame:
    ...
    keep_vars = [v for v in ["chl_model", "light_attenuation", "phyc"] if v in point.data_vars]
...
def main() -> None:
    ensure_dir(RAW_BGC_DIR); ensure_dir(FEATURES_DIR)
    log_kv("RAW_DIR", RAW_BGC_DIR); log_kv("PROCESSED_DIR", FEATURES_DIR); log_kv("SPOTS_FILE", SPOTS_FILE)
    ...
    nc_files = find_netcdf_files(RAW_BGC_DIR, years=YEARS)
    if not nc_files:
        if DOWNLOAD_IF_MISSING:
            print("Aucun fichier trouvé -> téléchargement Copernicus..."); download_original_files(); nc_files = find_netcdf_files(RAW_BGC_DIR, years=YEARS)
        else:
            raise FileNotFoundError(f"Aucun fichier .nc trouvé dans {RAW_BGC_DIR}. Place tes fichiers sous data/raw/bgc/, ou passe DOWNLOAD_IF_MISSING=True.")
    ds_bgc = open_and_crop_bgc(nc_files, lon_min, lon_max, lat_min, lat_max)
    df = build_spot_dataframe(ds_bgc, spots).sort_values(["spot_id", "date"]).reset_index(drop=True)
