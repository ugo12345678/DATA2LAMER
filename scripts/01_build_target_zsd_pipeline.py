...
from config.settings import RAW_DIR, TARGET_DIR, SPOTS_SOURCE_FILE
...
OUT_FILE = TARGET_DIR / f"target_zsd_daily_by_spot_{YEARS_LABEL}.parquet"
OUT_SAMPLE_FILE = TARGET_DIR / "target_zsd_daily_by_spot_sample.parquet"
OUT_SUMMARY_FILE = TARGET_DIR / "target_zsd_daily_by_spot_summary.csv"
...
def download_original_files() -> None:
    ...
    copernicusmarine.get(
        ...
        output_directory=str(RAW_ZSD_DIR),
        filter=YEAR_FILTER,
        overwrite=False,
...
def find_netcdf_files(folder: Path, years: list[int] | None = None) -> list[Path]:
    ...
    if years:
        ...
        files = [
            f for f in files
            if f.name[:4] in years_as_str or any(part in years_as_str for part in f.parts)
        ]
    return files
...
def open_and_crop_zsd(nc_files: list[Path], lon_min: float, lon_max: float, lat_min: float, lat_max: float) -> xr.Dataset:
    if not nc_files:
        raise FileNotFoundError("Aucun fichier .nc trouvé pour ZSD")
    ...
    for i, file_path in enumerate(nc_files, start=1):
        ...
        try:
            ...
            if VAR_NAME not in ds.data_vars:
                print(f"[ZSD] [{i}/{total}] variable {VAR_NAME} absente", flush=True)
                ds.close()
                continue
            ds = ds[[VAR_NAME]]
            ...
            if ds.sizes.get("lat", 0) == 0 or ds.sizes.get("lon", 0) == 0:
                print(f"[ZSD] [{i}/{total}] vide après découpe", flush=True)
                ds.close()
                continue
            print(f"[ZSD] [{i}/{total}] OK en {time.time() - t0:.1f}s | dims={dict(ds.sizes)}", flush=True)
            datasets.append(ds)
        except Exception as exc:
            print(f"[ZSD] [{i}/{total}] ERREUR en {time.time() - t0:.1f}s : {exc}", flush=True)

    if not datasets:
        raise ValueError("Aucun fichier exploitable pour ZSD")

    print(f"[ZSD] concaténation de {len(datasets)} sous-datasets...", flush=True)
    ds_merged = xr.concat(datasets, dim="time")
    ds_merged = ds_merged.sortby("time")
    ds_merged = ds_merged.drop_duplicates(dim="time")
    print(f"[ZSD] concat terminé | dims={dict(ds_merged.sizes)}", flush=True)
    return ds_merged
...
def extract_one_spot(ds: xr.Dataset, spot: pd.Series) -> pd.DataFrame:
    ...
    point_df = point[[VAR_NAME]].to_dataframe().reset_index()
    if point_df.empty:
        return point_df
    ...
    group_cols = [
        "date", "spot_id", "spot_name", "spot_lat", "spot_lon", "grid_lat", "grid_lon",
    ]
    for col in OPTIONAL_SPOT_COLS:
        if col in point_df.columns:
            group_cols.append(col)

    agg = point_df.groupby(group_cols, as_index=False)[VAR_NAME].mean()
...
def build_spot_dataframe(ds: xr.Dataset, spots: pd.DataFrame) -> pd.DataFrame:
    ...
    for i, (_, spot) in enumerate(spots.iterrows(), start=1):
        ...
        if df_spot.empty:
            print(f"[SPOT] [{i}/{total}] vide", flush=True)
            continue
        frames.append(df_spot)
    if not frames:
        raise ValueError("Aucune donnée spot extraite")
    df = pd.concat(frames, ignore_index=True)
    return df.rename(columns={VAR_NAME: "zsd"}).dropna(subset=["zsd"])


def main() -> None:
    ensure_dir(RAW_ZSD_DIR)
    ensure_dir(TARGET_DIR)

    log_kv("RAW_DIR", RAW_ZSD_DIR)
    log_kv("PROCESSED_DIR", TARGET_DIR)
    ...
    print(f"BBox spots : lon=({lon_min:.3f}, {lon_max:.3f}) lat=({lat_min:.3f}, {lat_max:.3f})")

    nc_files = find_netcdf_files(RAW_ZSD_DIR, years=YEARS)
    if not nc_files:
        if DOWNLOAD_IF_MISSING:
            print("Aucun fichier trouvé -> téléchargement Copernicus...")
            download_original_files()
            nc_files = find_netcdf_files(RAW_ZSD_DIR, years=YEARS)
        else:
            raise FileNotFoundError(
                f"Aucun fichier .nc trouvé dans {RAW_ZSD_DIR}. "
                "Place tes fichiers sous data/raw/zsd/, ou passe DOWNLOAD_IF_MISSING=True."
            )

    ds_zsd = open_and_crop_zsd(nc_files, lon_min, lon_max, lat_min, lat_max)
    df = build_spot_dataframe(ds_zsd, spots).sort_values(["spot_id", "date"]).reset_index(drop=True)
